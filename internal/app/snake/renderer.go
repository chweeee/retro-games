...
func renderArena(a *arena, top, bottom, left int) {
	// this loop draws the left/right borders
	for i := top; i < bottom; i++ {
		// why need to -1 on left-coord?
...
func renderTitle(left, top int) {
	// render the title at the center of the arena
	mid := left - len("Snake Game")/2
	tbprint(mid, top-1, defaultColor, defaultColor, "Snake Game")
}
...
// prints string in a line within each cell
...
func (g *Game) render() error {
	...
	renderTitle(left+g.arena.width/2, top)
	renderArena(g.arena, top, bottom, left) // get it to work with oob checking
